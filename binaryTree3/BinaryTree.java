...
public class BinaryTree {
    ...
    //根据中序和后序创建二叉树
    public  int index=0;
    private int indexOfTreeNode(char[] inorder, char val, int inbegin, int inend) {
        for (int i = inbegin; i <= inend; i++)
            if (inorder[i] == val)
                return i;
        return -1;
    }
    private TreeNode build(char[] postorder,char[] inorder, int inbegin, int inend) {
